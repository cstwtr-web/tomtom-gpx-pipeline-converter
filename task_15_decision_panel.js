...
import { isSemanticName }                from './task_04_waypoint_policy.js';
import { redistributeByDistance }        from './task_07_geometry.js';
import { motoOptimize }                  from './task_07_geometry.js';
import { nameWaypoints }                 from './task_06_geocoding_client.js';
...
export async function decisionWpApply() {
  ...
  if (origSrc === 'trkpt') {
    if (!rawTrk?.length) {
      _addLog('❌ Traccia GPS originale non disponibile (rawTrkPoints vuoti)', 'warn');
      _dpWpTarget = null;
      showDecisionPanel(); // ripristina applyBtn/minusBtn/plusBtn/hintEl — senza, restavano disabilitati per sempre
      return;
    }
