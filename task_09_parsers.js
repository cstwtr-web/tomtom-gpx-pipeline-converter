...
function _cleanStopName(stop) {
  if (!stop) return stop;
  // FIX v18.5 — Magenta IT vs FR:
  // La regex originale rimuoveva il CAP (es. "20013") che era l'unico elemento
  // che permetteva a _detectCountryCode() (task_05) di riconoscere la provincia
  // italiana tramite l'AND: /\b\d{5}\b/ && /\b(MI|TO|...)\b/.
  // Soluzione: prima della pulizia, catturiamo la sigla provincia IT se presente
  // nella forma canonica Google Maps "Città, 20013 Città MI". Se dopo la pulizia
  // la sigla è scomparsa (capita quando CAP e sigla erano nel blocco rimosso),
  // la riappende come suffisso → "_detectCountryCode" può fare il match solo su sigla.
  const _IT_PROVINCE_RE = /\b(AG|AL|AN|AO|AP|AQ|AR|AT|AV|BA|BG|BI|BL|BN|BO|BR|BS|BT|BZ|CA|CB|CE|CH|CI|CL|CN|CO|CR|CS|CT|CZ|EN|FC|FE|FG|FI|FM|FR|GE|GO|GR|IM|IS|KR|LC|LE|LI|LO|LT|LU|MB|MC|ME|MI|MN|MO|MS|MT|NA|NO|NU|OG|OR|OT|PA|PC|PD|PE|PG|PI|PN|PO|PR|PT|PU|PV|PZ|RA|RC|RE|RG|RI|RM|RN|RO|SA|SI|SO|SP|SR|SS|SV|TA|TE|TN|TO|TP|TR|TS|TV|UD|VA|VB|VC|VE|VI|VR|VS|VT|VV)\b/;
  const provinceMatch = stop.match(_IT_PROVINCE_RE);
  const cleaned = stop.replace(/,\s*\d{4,5}(?=\s*[,)]|$)/g, '').replace(/\s{2,}/g, ' ').trim();
  // Riappende la sigla solo se era presente e ora manca (es. era dentro il blocco CAP)
  if (provinceMatch && !_IT_PROVINCE_RE.test(cleaned)) {
    return (cleaned + ' ' + provinceMatch[1]).trim();
  }
  return cleaned;
}
...
/** Estrae tutte le coppie coordinate da una URL Google Maps blob.
 *  Supporta due formati:
 *  - !1d<lon>!2d<lat>  → URL desktop/espansa classica (ordine lon, lat)
 *  - !8m2!3d<lat>!4d<lon> → URL espansa da app mobile / short-link (ordine lat, lon)
 *  I risultati vengono deduplicati e restituiti nell'ordine di apparizione nell'URL.
 */
export function extractAllWaypointCoords(url) {
  const coords = [];
  let m;

  // Formato 1: !1d<lon>!2d<lat>  (URL desktop / espansa classica)
  const re1 = /!1d(-?\d+\.\d+)!2d(-?\d+\.\d+)/g;
  while ((m = re1.exec(url)) !== null)
    coords.push({ lat: parseFloat(m[2]), lon: parseFloat(m[1]), _idx: m.index });

  // Formato 2: !8m2!3d<lat>!4d<lon>  (URL da app mobile / short-link espanso)
  const re2 = /!8m2!3d(-?\d+\.\d+)!4d(-?\d+\.\d+)/g;
  while ((m = re2.exec(url)) !== null)
    coords.push({ lat: parseFloat(m[1]), lon: parseFloat(m[2]), _idx: m.index });

  // Ordina per posizione nell'URL (mantiene duplicati — necessari per mapping 1:1 con stops)
  coords.sort((a, b) => a._idx - b._idx);
...
/** Estrae coordinate dall'ultimo blob nella stringa.
 *  Supporta !1d!2d (lon/lat) e !8m2!3d!4d (lat/lon).
 */
export function blobCoords(s) {
  ...
  while ((m = re1.exec(s)) !== null)
    pairs.push({ lat: parseFloat(m[2]), lon: parseFloat(m[1]), _idx: m.index });

  const re2 = /!8m2!3d(-?\d+\.\d+)!4d(-?\d+\.\d+)/g;
  while ((m = re2.exec(s)) !== null)
    pairs.push({ lat: parseFloat(m[1]), lon: parseFloat(m[2]), _idx: m.index });

  if (!pairs.length) return null;
  pairs.sort((a, b) => b._idx - a._idx);          // ultimo per posizione
  const p = pairs[0];
...
// [CHECKP_TASK_07] hash: v18.6_blob_3d4d_mobile_url_fix
